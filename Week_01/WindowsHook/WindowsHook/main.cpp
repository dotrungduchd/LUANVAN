...
HINSTANCE hInst;
API_HOOK Hook, Hook2;
char szModuleName[260], str[1024];
	...
	)
{
	...
	/*TCHAR buffer[1024];
	sprintf_s(buffer, 1024, "CreateFileW: %ls + %ls", hdFile, lpFileName);
...
{
	MessageBox(0, LPSTR("NtDll"), LPSTR("NtCreateFile"),
		MB_ICONINFORMATION);
...
BOOL WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		sprintf(str, "Hook DLL loaded into process %s (%d)", szModuleName, GetCurrentProcessId());

		OutputDebugString(str);

		//if (strstr(szModuleName, "xplorer+.") == NULL) break;

		//InitAPIHook(&Hook,"wininet.dll","InternetConnectW",HookInternetConnectW);

		InitAPIHook(&Hook, "Kernel32.dll", "CreateFileW", HookCreateFile);
